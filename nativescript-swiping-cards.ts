...
export class CardContainer extends AbsoluteLayout {
	private currentPanel: ICardMap;
	...
	private showRightSlide(panelMap: ICardMap, offset: number = 0): AnimationModule.AnimationPromise {
	...
	private showLeftSlide(panelMap: ICardMap, offset: number = 0): AnimationModule.AnimationPromise {
		let transition = new Array();
		transition.push({
	...
	private buildSlideMap(views: StackLayout[]) {
		let slideMap: ICardMap[] = [];
		views.forEach((view: StackLayout) => {
			slideMap.push({
				panel: view
			});
		});
		slideMap.forEach((mapping: ICardMap, index: number) => {
